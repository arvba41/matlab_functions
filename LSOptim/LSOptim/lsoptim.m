...
% Levenberg-Marquardt like method. It calculates and approximation to the
% gradient using single side finite differences.
%
...
% Version 1.5
%  + Improved the algorithm that tries to ensure a descent by checking if
%  it gives an improvement over the iterations.
%  + Changed the printout, for the gradient calculations approximation.
...
if nargin > 5
    for ii = 6:nargin
        VarParString = [VarParString,',varargin{',num2str(ii-3),'}'] ;
    end
end
...
if nargin>3,
    ...
    if nargin==4,
        ...
        if isempty(Lin_A)||isempty(Lin_b),
            ...
            %  disp(['start '])
            %
            if max(Lin_A*parameters-Lin_b)<=0,
                disp(num2str(Lin_A*parameters-Lin_b))
...
while ((noSteps<maxSteps) & (abs((Vn_old-Vn)/Vn)>convergenceTol)) %&(dk'*dk>delta*parameters'*parameters*delta)),
    ...
    for ii=1:nvar,
        temp_par=parameters;
        deltaX=delta*abs(ParameterSize(ii));
        %if (deltaX<1e-14),
        %  deltaX=1e-14;
        %  [delta i]
        %end
        temp_par(ii)=parameters(ii)+deltaX;
        J(:,ii)=(eval(evalstring)-F)/deltaX;
        if isnan(J(:,ii)),
            error('Gradient returned NaN for Variable number %d.',ii);
        end
        if DispFlag,
            fprintf(1,'\b\b\b\b%3d%%',round(ii/nvar*100));
        end
    end
    lambda=min(Vn_best/5000,lambda);
    if Constrained,
        % ---- Run the log barrier penalty function
        H=J'*J+lambda*I+mu*Lin_A'*diag(1./max((Lin_A*parameters-Lin_b),eps).^2)*Lin_A;
        JJ=J'*F-mu*Lin_A'*(1./max((Lin_A*parameters-Lin_b),eps));
        warning('off')
        dk=-H\JJ;
        warning('on')
    ...
    if isnan(dk),
        disp((Lin_A*parameters-Lin_b)')
        error('Step returned NaN.');
    end
    ...
    while (alpha>1e-9),
        ...
        if (Vn>Vn_prev),
            ...
            if (Vn>Vn_stored),
                ...
                if Constrained,
                    H=J'*J+lambda/alpha*I+mu*Lin_A'*diag(1./max((Lin_A*parameters-Lin_b),eps).^2)*Lin_A;
                    JJ=J'*F-mu*Lin_A'*(1./max((Lin_A*parameters-Lin_b),eps));
                    warning('off')
                    dk=-H\JJ;
                    warning('on')                    
...
if isempty(t_limit) | (t_limit>(1+eps)),
    t=1;
else
    t=t_limit*.95;
    if DispFlag,
